...
function inferFieldType(name: string, schema: any): { type: FieldType; voice: boolean } {
  ...
  const longText =
    ...
    /bio|body|description|content|message|note|about|summary|rules/.test(n)
  if (longText)
...
function genListPage(name: string, hook: string, op: any): string {
  ...
  return `${reactImport}import { ${hook} } from '@streamyolo/sdk'
...
import { ${hook} } from '@streamyolo/sdk'
import { Card, CardContent } from '@/components/ui/Card'
import { Skeleton } from '@/components/ui/Skeleton'
...
import { z } from 'zod'
import { ${queryHook}, ${mutationHook} } from '@streamyolo/sdk'
import { Form } from '@/components/ui/Form'
import { Skeleton } from '@/components/ui/Skeleton'
import type { FieldConfig } from '@/components/ui/Form'
